...
const ChannelSidebar = ({ server, activeChannel, voiceChannelParticipants, onChannelSelect, onChannelCreated, onServerUpdate, onVoiceChannelJoin, currentVoiceChannel, isVoiceConnected, isMuted, isDeafened, user, onServerLeft }) => {
  // Categories state - Discord-like custom categories
  const [categories, setCategories] = useState([
    { id: 'text-default', name: 'TEXT CHANNELS', type: 'text', position: 0 },
    { id: 'voice-default', name: 'VOICE CHANNELS', type: 'voice', position: 1 }
  ]);
  const [expandedCategories, setExpandedCategories] = useState(new Set(["text-default", "voice-default"]));
  ...
  // Group channels by category (Discord-like)
  const getChannelsByCategory = () => {
    ...
    server?.channels?.forEach(channel => {
      const categoryId = channel.category || (channel.type === 'voice' ? 'voice-default' : 'text-default');
  ...
  return (
    <>
      <div className="w-72 bg-black/40 backdrop-blur-md border-r border-white/10 flex flex-col">
        ...
        {/* Channel List - Dynamic Categories */}
              ...
              <Collapsible key={category.id} open={isExpanded}>
                ...
                >
                  ...
                  {isOwner && (
                    <div
                      ...
                      className="w-6 h-6 p-1 opacity-0 group-hover:opacity-100 text-white hover:text-gray-200 transition-all cursor-pointer flex items-center justify-center rounded hover:bg-white/10"
                    >
                ...
                <CollapsibleContent className="space-y-1">
                  {channels.map((channel) => {
                    ...
                    const isActive = (activeChannel?._id || activeChannel?.id) === channelId;
                    ...
                    if (isVoice) {
                      ...
                      return (
                        <div key={channelId} className="space-y-1">
                          <div className="relative group">
                            ...
                            {isOwner && (
                              <div className="absolute right-2 top-1/2 -translate-y-1/2 z-10 pointer-events-none group-hover:pointer-events-auto">
                                <DropdownMenu>
                                  <DropdownMenuTrigger asChild>
                                    <Button variant="ghost" size="icon" className="w-8 h-8 text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity hover:text-gray-300 hover:bg-gray-700/30">
                                  ...
                                  <DropdownMenuContent align="end" className="bg-gray-800 border-gray-600 text-white">
                                    ...
                                    <DropdownMenuItem
                                      ...
                                      className="hover:bg-red-700 text-red-400 cursor-pointer"
                                    >
                          ...
                          {/* Voice Participants */}
                          <VoiceParticipantList channelId={channelId} showMuteIcons={true} />
                        </div>
                      );
                    } else {
